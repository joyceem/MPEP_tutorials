...
```
The observed states are denoted with either a 0 or a 1, while the hidden states are denoted with A and B. The numbers in the matrix (1-5) don’t tell us what the actual rates are, just that elements with the same number will share a rate in our model. Here, we have set it so that transitions from 1→0 can be different from transitions from 0→1, but that transitions between our hidden states are equal. We can also see a few NAs in our matrix: we see these whenever we see both states transition at once ( 1A↔0B or 0A↔1B). Generally, we assume that only one state changes at a given time so we want to exclude the possibility that both transitions occur at once.
